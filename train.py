...
parser.add_argument('--max_epoch', type=int, default=450, help='Epoch to run [default: 251]')
...
parser.add_argument('--checkpoint', default='False', help='model checkpoint file path [default: log/model.ckpt]') #change1
...
print("value of root is :",ROOT_DIR)
MODEL_FILE = os.path.join(ROOT_DIR, 'Deep_Annular_Residual_Feature_Learning_for_3dPointClouds/models', FLAGS.model+'.py')
print("value of model_file is :",MODEL_FILE)
...
if FLAGS.normal:
    ...
    ip=input("I am done with pre-processing, what to do netxt :?")
    DATA_PATH = os.path.join(DATA_DIR, '/home/rabbia/code/H5/repo/Deep_Annular_Residual_Feature_Learning_for_3dPointClouds/data/modelnet40_ply_hdf5_2048/modelnet40')
...
def train():
    with tf.Graph().as_default():
        ...
        best_acc = -1
        best_acc_avg = -1
        if FLAGS.checkpoint=='true':  #change#5 these 32 lines o code are added to load the check points and update the number of epochs accordingly.
            print("value inside if is :",FLAGS.checkpoint)
            check_point_dir='log/'
            val=tf.train.latest_checkpoint(check_point_dir)
            saver.restore(sess, val)
            print("please see it :",tf.train.latest_checkpoint(FLAGS.checkpoint))
            print("Yes I did rstored the model hahaha :")
            #print("FLAGS.checkpoint is :",FLAGS.checkpoint)
            print("FLAGS.checkpoint is :",val)
            #found_num = re.search(r'\d+', FLAGS.checkpoint)
            #found_num = re.search(r'\d+', val)
            #print("found num is ",found_num)
            #starting_epoch = val.split('-')[-1]
            #starting_epoch = val.split('-')
            #print("starting epoch is  ",starting_epoch[1])
            #found_num = re.search(r'\d+', val)
            #print("found num is :",found_num)
            ip=input("Next :?")
