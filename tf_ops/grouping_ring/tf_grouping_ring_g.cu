...
// input: points (b,n,c), idx (b,m,nsample)
// output: out (b,m,nsample,c)
__global__ void group_point_gpu3(int b, int n, int c,int d, int m, int nsample, const float *points, const int *idx, float *out) {
    int batch_index = blockIdx.x;
    //points += n*c*batch_index;
    //idx += m*nsample*batch_index;
    //out += m*nsample*c*batch_index;
     points += n*c*d*batch_index;
	 //idx += m*batch_index;
	 idx += m*nsample*batch_index;

    out += m*nsample*c*d*batch_index;
    int index = threadIdx.x;
    int stride = blockDim.x;
    ...
    for (int j=index;j<m;j+=stride) {
        for (int k=0;k<nsample;++k) {
            ...
            for (int l=0;l<h;++l) {
                //out[j*nsample*c+k*c+l] = points[ii*c+l];
				out[j*nsample*h+k*h+l] = points[ii*h+l];
            }
        }
    }
}
// input: grad_out (b,m,nsample,c,d), idx (b,m,nsample),
// output: grad_points (b,n,c,d)
__global__ void group_point_grad3_gpu(int b, int n, int c, int d,int m, int nsample, const float *grad_out, const int *idx, float *grad_points) {
    int batch_index = blockIdx.x;
    idx += m*nsample*batch_index;
	//grad_out for the above grouppointgrad op is same as that of the output of the group_pointop so we are replicating that
    //grad_out += m*nsample*c*batch_index;
	//out += m*nsample*c*d*batch_index; //out of the op group_point3
	grad_out += m*nsample*c*d*batch_index;
    //grad_points += n*c*batch_index;
    grad_points += n*c*d*batch_index;
    int index = threadIdx.x;
    int stride = blockDim.x;
...
for (int j=index;j<m;j+=stride) {
        for (int k=0;k<nsample;++k) {
            ...
            for (int l=0;l<h;++l) {
				...
				//out[j*nsample*h+k*h+l] = points[ii*h+l];
				atomicAdd(&grad_points[ii*h+l], grad_out[j*nsample*h+k*h+l]);
            }
        }
    }
}
...
void groupPointLauncher3(int b, int n, int c,int d, int m,int nsample, const float *points, const int *idx, float *out){
    group_point_gpu3<<<b,256>>>(b,n,c,d,m,nsample,points,idx,out);
...
void groupPointGradLauncher3(int b, int n, int c, int d,int m, int nsample, const float *grad_out, const int *idx, float *grad_points){
    group_point_grad3_gpu<<<b,256>>>(b,n,c,d,m,nsample,grad_out,idx,grad_points);
    //cudaDeviceSynchronize();
}
