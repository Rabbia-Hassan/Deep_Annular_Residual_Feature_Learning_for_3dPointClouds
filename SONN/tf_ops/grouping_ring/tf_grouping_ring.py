...
def group_point3(points, idx):
    ...
    Output:
        out: (batch_size, npoint, nsample, channel) float32 array, values sampled from points
    '''
    return grouping_ring_module.group_point3(points, idx)
#@tf.RegisterGradient('GroupPoint3')
@tf.RegisterGradient('GroupPoint3')
#ops.NoGradient('GroupPoint3')
def _group_point_grad3(op, grad_out):
    points = op.inputs[0]
    idx = op.inputs[1]
    return [grouping_ring_module.group_point_grad3(points, idx, grad_out), None]
