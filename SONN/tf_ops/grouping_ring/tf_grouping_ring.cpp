...
void groupPointLauncher3(int b, int n, int c, int d,int m, int nsample, const float *points, const int *idx, float *out);
class GroupPointGpuOp3: public OpKernel{
    public:
        explicit GroupPointGpuOp3(OpKernelConstruction * context):OpKernel(context){}

        void Compute(OpKernelContext * context) override {
            const Tensor& points_tensor=context->input(0);
            OP_REQUIRES(context, points_tensor.dims()==4, errors::InvalidArgument("GroupPoint expects (batch_size, num_points, channel) points shape"));
            int b = points_tensor.shape().dim_size(0);
            int n = points_tensor.shape().dim_size(1);
            int c = points_tensor.shape().dim_size(2);
            int d = points_tensor.shape().dim_size(3);
            const Tensor& idx_tensor=context->input(1);
            OP_REQUIRES(context,idx_tensor.dims()==3 && idx_tensor.shape().dim_size(0)==b, errors::InvalidArgument("GroupPoint expects (batch_size, npoints, nsample) idx shape"));
            int m = idx_tensor.shape().dim_size(1);
            int nsample = idx_tensor.shape().dim_size(2);
            ...
            Tensor * out_tensor = nullptr;
            //OP_REQUIRES_OK(context, context->allocate_output(0,TensorShape{b,m,nsample,c}, &out_tensor));
            OP_REQUIRES_OK(context, context->allocate_output(0,TensorShape{b,m,nsample,c,d}, &out_tensor));
            auto points_flat = points_tensor.flat<float>();
            const float *points = &(points_flat(0));
            auto idx_flat = idx_tensor.flat<int>();
            const int *idx = &(idx_flat(0));
            auto out_flat = out_tensor->flat<float>();
            float *out = &(out_flat(0));
            groupPointLauncher3(b,n,c,d,m,nsample,points,idx,out);
        }
};
REGISTER_KERNEL_BUILDER(Name("GroupPoint3").Device(DEVICE_GPU),GroupPointGpuOp3);

void groupPointGradLauncher3(int b, int n, int c,int d, int m, int nsample, const float *grad_out, const int *idx, float *grad_points);
class GroupPointGradGpuOp3: public OpKernel{
    public:
        explicit GroupPointGradGpuOp3(OpKernelConstruction * context):OpKernel(context){}
        ...
        void Compute(OpKernelContext * context) override {
            ...
            //int c = points_tensor.shape().dim_size(2);
            const Tensor& points_tensor=context->input(0);
            OP_REQUIRES(context, points_tensor.dims()==4, errors::InvalidArgument("GroupPointGrad expects (batch_size, num_points, channel) points shape"));
            int b = points_tensor.shape().dim_size(0);
            int n = points_tensor.shape().dim_size(1);
            int c = points_tensor.shape().dim_size(2);
            int d = points_tensor.shape().dim_size(3);
            //idx_tensor is same as that of group_point i.e. it is the same 3d tensor of form b*1*nsample
			const Tensor& idx_tensor=context->input(1);
            OP_REQUIRES(context,idx_tensor.dims()==3 && idx_tensor.shape().dim_size(0)==b, errors::InvalidArgument("GroupPointGrad expects (batch_size, npoints, nsample) idx shape"));
            int m = idx_tensor.shape().dim_size(1);
            int nsample = idx_tensor.shape().dim_size(2);
            ...
            const Tensor& grad_out_tensor=context->input(2);
            //OP_REQUIRES(context,grad_out_tensor.dims()==4 && grad_out_tensor.shape().dim_size(0)==b && grad_out_tensor.shape().dim_size(1)==m && grad_out_tensor.shape().dim_size(2)==nsample && grad_out_tensor.shape().dim_size(3)==c, errors::InvalidArgument("GroupPointGrad expects (batch_size, npoints, nsample, channel) grad_out shape"));
            //basically the iutput of grouppoint'3 dimentions are being used here which are 5.
			OP_REQUIRES(context,grad_out_tensor.dims()==5 && grad_out_tensor.shape().dim_size(0)==b && grad_out_tensor.shape().dim_size(1)==m && grad_out_tensor.shape().dim_size(2)==nsample && grad_out_tensor.shape().dim_size(3)==c && grad_out_tensor.shape().dim_size(4)==d, errors::InvalidArgument("GroupPointGrad expects (batch_size, npoints, nsample, channel) grad_out shape"));
            
			Tensor * grad_points_tensor = nullptr;
            //OP_REQUIRES_OK(context, context->allocate_output(0,TensorShape{b,n,c}, &grad_points_tensor));
            OP_REQUIRES_OK(context, context->allocate_output(0,TensorShape{b,n,c,d}, &grad_points_tensor));
            auto points_flat = points_tensor.flat<float>();
            const float *points = &(points_flat(0));
            auto idx_flat = idx_tensor.flat<int>();
            const int *idx = &(idx_flat(0));
            ...
            float *grad_points = &(grad_points_flat(0));
            //cudaMemset(grad_points, 0, sizeof(float)*b*n*c);
			cudaMemset(grad_points, 0, sizeof(float)*b*n*c*d);
            groupPointGradLauncher3(b,n,c,d,m,nsample,grad_out,idx,grad_points);
        }
};
REGISTER_KERNEL_BUILDER(Name("GroupPointGrad3").Device(DEVICE_GPU),GroupPointGradGpuOp3);
